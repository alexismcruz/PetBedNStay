    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
      <div className="text-center mb-8">
        ...
        <p className="mt-3 text-stone-500 text-lg">
          Get your pet hotel or boarding service in front of thousands of pet owners — completely free.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        {[
          ...
          { icon: "📍", label: "Map Included" },
        ].map((b) => (
          <div key={b.label} className="text-center bg-white border border-amber-100 rounded-xl p-3">
            <div className="text-2xl mb-1">{b.icon}</div>
            <div className="text-xs font-semibold text-stone-600">{b.label}</div>
          </div>
        ))}
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-amber-100 shadow-sm p-6 sm:p-8 space-y-5">
        ...
        <p className="text-xs text-stone-400 text-center">
          ...
          <a href="/premium" className="text-brand-600 hover:underline">See Premium plans</a>.
        </p>
      </form>
    </div>
  );
